document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(requests&&window.chartPayload)new Chart(requests,{type:'bar',data:{labels:window.chartPayload.labels,datasets:window.chartPayload.datasets},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}},scales:{y:{beginAtZero:true,}}}});
  const spend=document.getElementById('articleSpendChart');
  if(spend&&window.articleSpend)new Chart(spend,{type:'bar',data:{labels:window.articleSpend.labels,datasets:[{label:'Quantidade',data:window.articleSpend.data,backgroundColor:['#1062fe','#12b76a','#f79009','#7a5af8','#f04438','#06aed4']}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{beginAtZero:true,}}}});
});
