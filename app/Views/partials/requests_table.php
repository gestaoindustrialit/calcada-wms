    ...
    <table class="table modern-table align-middle">
        ...
        <tbody>
            <?php foreach($groupedRequests as $r):
                ...
                $visibleLines = array_slice($r['lines'], 0, 3);
            ...
            ?>
                <tr>
                    ...
                    <td><div class="request-items-list"><?php foreach($visibleLines as $line): ?><span><strong><?= htmlspecialchars(($line['designation'] ?? '') ?: $line['item']) ?></strong> × <?= htmlspecialchars($line['quantity']) ?></span><?php endforeach; ?><?php if ($hiddenLinesCount > 0): $itemsToggleId = 'request-items-toggle-' . (int)$r['id']; ?><input type="checkbox" class="request-items-list__toggle" id="<?= $itemsToggleId ?>" aria-label="Mostrar ou ocultar <?= (int)$hiddenLinesCount ?> artigo(s) adicionais"><div class="request-items-list__extra"><?php foreach(array_slice($r['lines'], count($visibleLines)) as $line): ?><span><strong><?= htmlspecialchars(($line['designation'] ?? '') ?: $line['item']) ?></strong> × <?= htmlspecialchars($line['quantity']) ?></span><?php endforeach; ?></div><label class="request-items-list__more" for="<?= $itemsToggleId ?>" title="<?= htmlspecialchars('Mostrar ou ocultar ' . $hiddenLinesCount . ' artigo(s)') ?>"><span aria-hidden="true">…</span><span class="visually-hidden">Mostrar ou ocultar artigos adicionais</span></label><?php endif; ?></div></td>
