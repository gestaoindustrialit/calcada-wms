...
<div class="modal fade" id="<?= $modalId ?>" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content deliver-modal"><div class="modal-header"><div><span class="eyebrow">Entrega parcial</span><h5 class="modal-title">Entregar requisição</h5></div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button></div><div class="modal-body"><p class="text-muted mb-3">Confirme a entrega por artigo. Cada linha abate stock no respetivo armazém.</p><?php foreach($r['lines'] as $line): $lineRemaining=max(0,(float)$line['quantity']-(float)($line['delivered_quantity']??0)); ?><form method="post" action="<?= \App\Core\Url::page('request_action') ?>&id=<?= $line['id'] ?>&do=deliver" class="delivery-line"><div><strong><?= htmlspecialchars($line['item']) ?></strong><small><?= htmlspecialchars($line['warehouse'] ?? '-') ?> · faltam <?= $lineRemaining ?></small></div><input class="form-control" name="deliver_quantity" type="number" min="0.01" max="<?= $lineRemaining ?>" step="0.01" value="<?= $lineRemaining ?>" required><button class="btn btn-primary" <?= $lineRemaining<=0 || $line['status'] === 'Cancelado'?'disabled':'' ?>><i class="bi bi-check2-circle"></i></button></form><?php endforeach; ?></div></div></div></div>
<?php endforeach; ?>
