        ...
        <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td><div class="request-items-list" data-request-items><?php foreach($r['lines'] as $lineIndex => $line): ?><span <?= $lineIndex >= count($visibleLines) ? 'data-request-item-extra hidden' : '' ?>><strong><?= htmlspecialchars($line['item']) ?></strong> × <?= htmlspecialchars($line['quantity']) ?></span><?php endforeach; ?><?php if ($hiddenLinesCount > 0): ?><button type="button" class="request-items-list__more" data-request-items-toggle aria-expanded="false" aria-label="Mostrar mais <?= (int)$hiddenLinesCount ?> artigo(s)" title="<?= htmlspecialchars('Mostrar mais ' . $hiddenLinesCount . ' artigo(s)') ?>">…</button><?php endif; ?></div></td>
