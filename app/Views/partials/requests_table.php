...
$stockByItemWarehouse = [];
foreach (($stockLocations ?? []) as $stock) {
    $stockByItemWarehouse[$stock['item_id'] . '-' . $stock['warehouse_id']][] = $stock;
}
$groupedRequests = [];
    ...
    <table class="table modern-table align-middle">
        ...
        <tbody>
            <?php foreach($groupedRequests as $r):
                ...
                $visibleLines = $r['lines'];
                $hiddenLinesCount = max(0, count($r['lines']) - 3);
            ?>
                <tr>
                    ...
                    <td><div class="request-items-list" data-request-items><?php foreach($visibleLines as $lineIndex => $line): $itemDesignation = trim((string)($line['designation'] ?? '')); $hasDesignation = $itemDesignation !== '' && strcasecmp($itemDesignation, trim((string)$line['item'])) !== 0; ?><span <?= $lineIndex >= 3 ? 'data-request-item-extra hidden' : '' ?>><strong><?= htmlspecialchars($hasDesignation ? $itemDesignation : 'Sem designação') ?></strong> × <?= htmlspecialchars($line['quantity']) ?></span><?php endforeach; ?><?php if ($hiddenLinesCount > 0): ?><button type="button" class="request-items-list__more" data-request-items-toggle aria-expanded="false" aria-label="Mostrar todos os artigos" title="Mostrar artigos adicionais">…</button><?php endif; ?></div></td>
...
<div class="modal fade" id="<?= $modalId ?>" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content deliver-modal"><div class="modal-header"><div><span class="eyebrow">Entrega parcial</span><h5 class="modal-title">Entregar requisição</h5></div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button></div><div class="modal-body"><p class="text-muted mb-3">Confirme a quantidade e a posição de stock de cada artigo.</p><form method="post" action="<?= \App\Core\Url::page('request_action') ?>&id=<?= $r['id'] ?>&do=deliver_all" class="delivery-form"><?php $hasDeliverableLines = false; foreach($r['lines'] as $line): $lineRemaining=max(0,(float)$line['quantity']-(float)($line['delivered_quantity']??0)); $positions=$stockByItemWarehouse[$line['item_id'].'-'.$line['warehouse_id']] ?? []; $lineDeliverable = $lineRemaining > 0 && $line['status'] !== 'Cancelado' && !empty($positions); $hasDeliverableLines = $hasDeliverableLines || $lineDeliverable; $itemDesignation = trim((string)($line['designation'] ?? '')); $hasDesignation = $itemDesignation !== '' && strcasecmp($itemDesignation, trim((string)$line['item'])) !== 0; ?><div class="delivery-line"><div><strong><?= htmlspecialchars($hasDesignation ? $itemDesignation : 'Sem designação') ?></strong><small><?= htmlspecialchars($line['warehouse'] ?? '-') ?> · faltam <?= $lineRemaining ?></small></div><select class="form-select" name="deliver_locations[<?= (int)$line['id'] ?>]" aria-label="Posição de stock" required <?= $lineDeliverable ? '' : 'disabled' ?>><?php if (!$positions): ?><option value="">Sem stock disponível</option><?php else: foreach($positions as $position): ?><option value="<?= htmlspecialchars($position['location']) ?>"><?= htmlspecialchars(($position['location'] !== '' ? $position['location'] : 'Sem posição') . ' · stock ' . $position['quantity']) ?></option><?php endforeach; endif; ?></select><input class="form-control" name="deliver_quantities[<?= (int)$line['id'] ?>]" type="number" min="0.01" step="0.01" value="<?= $lineRemaining ?>" required <?= $lineDeliverable ? '' : 'disabled' ?>><button type="submit" name="request_action" value="deliver" class="btn btn-primary" formaction="<?= \App\Core\Url::page('request_action') ?>&id=<?= $line['id'] ?>&do=deliver" formmethod="post" <?= $lineDeliverable?'':'disabled' ?> title="Validar esta quantidade" aria-label="Validar esta quantidade"><i class="bi bi-check2-circle"></i></button></div><?php endforeach; ?><div class="delivery-actions"><button type="submit" name="request_action" value="deliver_all" class="btn btn-primary w-100" formaction="<?= \App\Core\Url::page('request_action') ?>&id=<?= $r['id'] ?>&do=deliver_all" formmethod="post" <?= $hasDeliverableLines ? '' : 'disabled' ?>><i class="bi bi-check2-all me-2"></i>Validar todas as quantidades</button></div></form></div></div></div></div>
<?php endforeach; ?>
