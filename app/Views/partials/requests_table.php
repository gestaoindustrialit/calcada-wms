        ...
        <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td><div class="request-items-list"><?php foreach($visibleLines as $line): $itemDesignation = trim((string)($line['designation'] ?? '')); $hasDesignation = $itemDesignation !== '' && strcasecmp($itemDesignation, trim((string)$line['item'])) !== 0; ?><span><strong><?= htmlspecialchars($hasDesignation ? $itemDesignation : 'Sem designação') ?></strong> × <?= htmlspecialchars($line['quantity']) ?></span><?php endforeach; ?><?php if ($hiddenLinesCount > 0): ?><span class="request-items-list__more" title="<?= htmlspecialchars($hiddenLinesCount . ' artigo(s) adicional(is)') ?>">…</span><?php endif; ?></div></td>
