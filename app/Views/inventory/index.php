...
<form method="post" action="<?= \App\Core\Url::page('inventory_save') ?>" class="row g-2 quick-form mb-4" data-inventory-movement-form data-inventory-locations='<?= htmlspecialchars(json_encode($inventoryLocationPayload, JSON_UNESCAPED_UNICODE), ENT_QUOTES, 'UTF-8') ?>'>
...
<form method="get" class="quick-form mb-4">
    ...
    <div class="row g-2 align-items-end">
        ...
        <div class="col-lg-3"><label class="form-label">Artigo</label><select class="form-select searchable-select" name="item_id" data-search-placeholder="Pesquisar artigo"><option value="">Todos os artigos</option><?php foreach($items as $i): ?><option value="<?= $i['id'] ?>" <?= (int)($filters['item_id'] ?? 0) === (int)$i['id'] ? 'selected' : '' ?>><?= htmlspecialchars($i['name']) ?></option><?php endforeach; ?></select></div>
