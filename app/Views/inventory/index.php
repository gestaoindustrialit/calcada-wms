...
<form method="get" class="quick-form mb-4">
    ...
    <div class="row g-2 align-items-end">
        ...
        <div class="col-lg-3"><label class="form-label">Artigo</label><select class="form-select" name="item_id"><option value="">Todos os artigos</option><?php foreach($items as $i): ?><option value="<?= $i['id'] ?>" <?= (int)($filters['item_id'] ?? 0) === (int)$i['id'] ? 'selected' : '' ?>><?= htmlspecialchars($i['name']) ?></option><?php endforeach; ?></select></div>
        <div class="col-lg-2"><label class="form-label">Armazém</label><select class="form-select" name="warehouse_id"><option value="">Todos</option><?php foreach($warehouses as $w): ?><option value="<?= $w['id'] ?>" <?= (int)($filters['warehouse_id'] ?? 0) === (int)$w['id'] ? 'selected' : '' ?>><?= htmlspecialchars($w['name']) ?></option><?php endforeach; ?></select></div>
...
<<<<<<< codex/adicionar-filtragem-ao-inventario-1fpq34
...
=======
<div class="table-responsive data-shell"><table class="table align-middle"><thead><tr><th>Artigo</th><th>Designação</th><th>Armazém</th><th>Qtd</th><th>Mín.</th><th>Valor</th><th>Ações</th></tr></thead><tbody><?php foreach($rows as $r): ?><tr class="<?= $r['quantity'] <= $r['min_quantity'] ? 'table-warning' : '' ?>"><td><?= htmlspecialchars($r['item']) ?></td><td><?= htmlspecialchars($r['designation'] ?? '') ?></td><td><?= htmlspecialchars($r['warehouse']) ?></td><td><strong><?= htmlspecialchars($r['quantity'].' '.$r['unit']) ?></strong></td><td><?= htmlspecialchars($r['min_quantity']) ?></td><td>€ <?= number_format((float)$r['stock_value'],2,',','.') ?></td><td><a class="btn btn-sm btn-outline-primary" href="<?= \App\Core\Url::page('inventory') ?>&edit=<?= $r['id'] ?>" title="Editar" aria-label="Editar"><i class="bi bi-pencil"></i></a> <a class="btn btn-sm btn-outline-danger" href="<?= \App\Core\Url::page('inventory') ?>&delete=<?= $r['id'] ?>" onclick="return confirm('Apagar stock?')" title="Apagar" aria-label="Apagar"><i class="bi bi-trash"></i></a></td></tr><?php endforeach; ?></tbody></table></div>
>>>>>>> main
