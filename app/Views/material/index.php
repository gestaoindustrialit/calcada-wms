...
<thead><tr><th>Estado</th><th>Criado</th><th>Responsável</th><th>Departamento</th><th>Produto</th><th>Operação</th><th>Qtd</th><th>Concl.</th><th>Urg.</th><th>Entrega</th><th>Obs.</th><th>Fich.</th><th>Fat.</th><?php if($canActOnMaterial): ?><th>Ações</th><?php endif; ?></tr></thead>
<tbody>
<?php foreach($rows as $row): ?>
<tr>
<td><span class="badge status-badge material-status"><?= htmlspecialchars($row['status']) ?></span></td><td><?= htmlspecialchars(date('d/m/Y H:i', strtotime($row['created_at']))) ?></td><td><?= htmlspecialchars($row['responsible']) ?></td><td><span class="pill"><?= htmlspecialchars($row['department']) ?></span></td><td><?= htmlspecialchars($row['product']) ?></td><td><?= htmlspecialchars($row['operation']) ?></td><td><?= htmlspecialchars($row['quantity']) ?></td><td><?= htmlspecialchars($row['completed_quantity']) ?></td><td><?= str_repeat('<i class="bi bi-flag"></i>', (int)$row['urgency']) ?></td><td><?= htmlspecialchars(date('d/m/Y', strtotime($row['due_date']))) ?></td><td class="material-notes-cell"><?= htmlspecialchars($row['notes'] ?? '') ?></td><td class="text-center"><?php if(!empty($row['attachment_path'])): ?><a class="btn btn-light btn-sm" href="<?= Url::page('material_download') ?>&id=<?= (int)$row['id'] ?>" title="Descarregar <?= htmlspecialchars($row['attachment_name'] ?? 'ficheiro') ?>"><i class="bi bi-download"></i></a><?php endif; ?></td><td class="text-center"><?php if(!empty($row['billed'])): ?><i class="bi bi-check-circle-fill text-success" title="Faturado"></i><?php endif; ?></td>
<?php if($canActOnMaterial): ?><td><form method="post" action="<?= Url::page('material_status') ?>" class="material-status-form"><input type="hidden" name="id" value="<?= (int)$row['id'] ?>"><?php if($canManageMaterial): ?><select class="form-select form-select-sm" name="status"><?php foreach($statusOptions as $status): ?><option value="<?= htmlspecialchars($status) ?>" <?= $row['status']===$status?'selected':'' ?>><?= htmlspecialchars($status) ?></option><?php endforeach; ?></select><input class="form-control form-control-sm" name="completed_quantity" type="number" min="0" step="0.01" value="<?= htmlspecialchars($row['completed_quantity']) ?>" title="Quantidade concluída"><?php else: ?><input type="hidden" name="status" value="<?= htmlspecialchars($row['status']) ?>"><input type="hidden" name="completed_quantity" value="<?= htmlspecialchars($row['completed_quantity']) ?>"><?php endif; ?><?php if($canEditMaterialDetails): ?><input class="form-control form-control-sm" name="due_date" type="date" value="<?= htmlspecialchars($row['due_date']) ?>" title="Data de entrega"><input class="form-control form-control-sm material-notes-input" name="notes" value="<?= htmlspecialchars($row['notes'] ?? '') ?>" title="Observações" placeholder="Observações"><?php endif; ?><?php if($canInvoiceMaterial): ?><label class="material-billed-check"><input class="form-check-input" type="checkbox" name="billed" value="1" <?= !empty($row['billed']) ? 'checked' : '' ?>> <span>Faturado</span></label><?php endif; ?><button class="btn btn-primary btn-sm" title="Guardar"><i class="bi bi-check-lg"></i></button></form></td><?php endif; ?>
</tr>
<?php endforeach; ?>
