    ...
    <article class="log-card <?= (int)$r['reverted'] === 1 ? 'is-reverted' : '' ?>">
        <div class="log-card__main">
            ...
            <div class="log-card__content">
                ...
                <div class="log-meta"><span><strong>Utilizador</strong><?= htmlspecialchars(trim(($r['user_name'] ?: 'Sistema') . ' ' . ($r['user_role'] ? '(' . $r['user_role'] . ')' : ''))) ?></span><span><strong>Data</strong><?= htmlspecialchars(date('d/m/Y H:i:s', strtotime($r['created_at']))) ?></span><?php if ((int)$r['reverted'] === 1): ?><span><strong>Anulado</strong><?= htmlspecialchars($r['reverted_at'] ?? '') ?></span><?php endif; ?></div>
                <details class="log-details"><summary>Ver todos os detalhes</summary><div class="log-diff-grid"><?php foreach($keys as $key): ?><div class="log-diff-row"><span><?= htmlspecialchars($fieldLabels[$key] ?? $key) ?></span><del data-label="Antes"><?= htmlspecialchars($fmt($before[$key] ?? null)) ?></del><ins data-label="Depois"><?= htmlspecialchars($fmt($after[$key] ?? null)) ?></ins></div><?php endforeach; ?><?php if(!$keys): ?><p class="text-muted mb-0">Sem campos alterados para apresentar.</p><?php endif; ?></div></details>
            </div>
        </div>
        <div class="log-card__aside">
            ...
            <?php if ((int)$r['reverted'] !== 1): ?><form method="post" action="<?= Url::page('log_action') ?>"><input type="hidden" name="id" value="<?= (int)$r['id'] ?>"><input type="hidden" name="log_action" value="revert"><button class="btn btn-sm btn-outline-danger w-100" onclick="return confirm('Anular esta modificação?')">Anular</button></form><?php endif; ?>
        </div>
    </article>
<?php endforeach; ?>
